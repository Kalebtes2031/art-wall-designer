...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${
      isScrolled
        ? 'bg-white/90 backdrop-blur-md shadow-md py-2 h-20'
        : 'bg-gradient-to-r from-blue-50 to-indigo-50 py-3 h-20'
    }`}>
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          {role === 'admin' && (
            <Link 
              to="/admin"
              className={`relative font-medium transition-colors ${
                location.pathname === '/admin' ? 'text-blue-600' : 'text-gray-700 hover:text-blue-500'
              }`}
            >
              Admin
      ...
      {/* Mobile menu */}
      <div className={`md:hidden bg-white shadow-lg rounded-b-xl transition-all duration-300 overflow-hidden ${
        ...
        <div className="px-2 pt-2 pb-3 space-y-1">
          ...
          {role === 'admin' && (
            <Link to="/admin" className={`block px-3 py-2 rounded-md text-base font-medium ${
              location.pathname === '/admin' ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50 hover:text-blue-600'
            }`}>
              Admin
            </Link>
          )}
          <div className="pt-4 border-t border-gray-200">
            {token ? (
              <button onClick={logout} className="w-full px-4 py-2 rounded-md bg-gradient-to-r from-blue-500 to-purple-500 text-white font-medium shadow">
                Logout
              </button>
            ) : (
              <>
                <Link to="/login" className="block w-full px-4 py-2 rounded-md bg-gradient-to-r from-blue-500 to-purple-500 text-white font-medium shadow mb-2">
                  Login
                </Link>
                <Link to="/signup" className="block w-full px-4 py-2 rounded-md bg-gradient-to-r from-blue-500 to-purple-500 text-white font-medium shadow">
                  Sign Up
                </Link>
              </>
            )}
