...
interface PlacedItemsContextValue {
  ...
  restoreFromLocalStorage: (dimensions: { width: number; height: number }) => void;
  mergeGuestItems: (backendAdd: any, dimensions: { width: number; height: number }) => void;
  setProductsRef: (products: Product[]) => void;
clearPlaced: () => void;
}

const PlacedItemsContext = createContext<PlacedItemsContextValue | undefined>(undefined);
...
export const PlacedItemsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const restoreFromLocalStorage = useCallback(
    (dimensions: { width: number; height: number }) => {
      try {
        ...
        const restored: PlacedItem[] = JSON.parse(saved).map((p: PlacedItem) => {
          const product = productsRef.current.find((prod) => prod._id === p.productId);
          const size = product?.sizes[p.sizeIndex];
          if (!size) return p;

          const { width, height } = calculateDimensions(size, dimensions.width);
          return {
            ...p,
  ...
  const addItem = async (
    ...
    const newId = `${item.productId}-${item.sizeIndex}-${Date.now()}`;
    ...
    if (backendAdd) {
      try {
        ...
        const updatedCart = await backendAdd(newItem.productId, newItem.sizeIndex, placement);
        if (updatedCart) {
          const matched = updatedCart.items.find(
            (i: CartItem) => i.product._id === newItem.productId && i.sizeIndex === newItem.sizeIndex
          );
          if (matched) {
            setPlaced((prev) =>
              prev.map((p) =>
                p.id === newId
                  ? { ...p, id: `${matched._id}-${p.sizeIndex}`, itemId: matched._id }
                  : p
              )
            );
          }
...
const moveItem = useCallback(
  ...
  const deleteItem = (id: string, backendDelete?: any) => {
    ...
    if (backendDelete && item?.itemId) {
      backendDelete(item.itemId).catch((err: any) => console.error("Failed backend delete:", err));
    }
  };
  ...
  const editItemSize = async (
    ...
    const scale = width / dimensions.width; // base scale for new size
  ...
  (cart: { items: CartItem[] } | undefined, dimensions: { width: number; height: number }) => {
    if (!cart) return;

    const restored = cart.items.map((item) => {
  const size = item.product.sizes[item.sizeIndex];
  const { width: baseWidth, height: baseHeight } = calculateDimensions(size, dimensions.width);
  const scale = item.scale ?? 1;

  // relative positions from backend
  const posX = item.positionX ?? 0.5;
  const posY = item.positionY ?? 0.5;

  // Convert relative position to canvas coordinates, considering item size
  const x = posX * dimensions.width - (baseWidth * scale) / 2;
  const y = posY * dimensions.height - (baseHeight * scale) / 2;
  ...
  const mergeGuestItems = useCallback(
    (backendAdd: any, dimensions: { width: number; height: number }) => {
      const guestItems = latestRef.current.filter((p) => !p.itemId);
      if (!guestItems.length) return;

      guestItems.forEach(async (guest) => {
        const alreadyMerged = latestRef.current.find(
          (p) => p.productId === guest.productId && p.sizeIndex === guest.sizeIndex && p.itemId
        );
        ...
        try {
          ...
          const updatedCart = await backendAdd(guest.productId, guest.sizeIndex, placement);
          if (!updatedCart) return;

          const matched = updatedCart.items.find(
            (i: CartItem) => i.product._id === guest.productId && i.sizeIndex === guest.sizeIndex
          );
...
export const usePlacedItems = () => {
  ...
  if (!ctx) throw new Error("usePlacedItems must be used within PlacedItemsProvider");
  return ctx;
};
